# UI layers (design system + Storybook)
...
app/**/page.tsx          → data, auth, redirects (no heavy markup)
  └── *-view.tsx         → pure UI props (storyable, layout: fullscreen)
...
4. **Pages for Storybook**: extract `FooView` with props; `page.tsx` loads data and renders `<FooView … />`. Story the view, not the async page + Supabase.
5. **Stories**: `components/ui/*.stories.tsx` for primitives; colocated `*.stories.tsx` next to views. Use `layout: "fullscreen"` for page chrome.
6. **E2E**: login, invite, material flows with real env — Playwright, not Storybook.
7. **New shadcn pieces**: `pnpm dlx shadcn@latest add <name>` → then Story if it’s public UI. Check Storybook MCP / existing stories before inventing props.

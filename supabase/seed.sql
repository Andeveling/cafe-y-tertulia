...
--
-- Crea/actualiza el miembro de empresa andresparra.nojau@gmail.com como 'active'
-- Password local: cafe1234  (cámbialo en Supabase Studio si querés)
-- Idempotente: podés correrlo N veces sin duplicar.

do $$
declare
  target_email text := 'andresparra.nojau@gmail.com';
  target_password text := 'cafe1234';
  target_name text := 'Andrés Parra';
...
begin
  ...
  if existing_id is null then
    ...
    insert into auth.users (
      ...
      instance_uuid, new_user_id, 'authenticated', 'authenticated', target_email, crypt(target_password, gen_salt('bf')),
      now(), '', '', '', '',
    ...
    values (new_user_id, 'active', target_name)
    on conflict (id) do update set status = 'active', display_name = excluded.display_name;

    raise notice 'Seed: creado % (%) con password %', target_email, new_user_id, target_password;
  else
    -- Ya existe: asegúrate que pueda loguearse y sea active
    update auth.users
      set encrypted_password = crypt(target_password, gen_salt('bf')),
          email_confirmed_at = coalesce(email_confirmed_at, now()),
          raw_user_meta_data = coalesce(raw_user_meta_data, '{}'::jsonb) || jsonb_build_object('display_name', target_name),
          updated_at = now()
    ...
    values (
      ...
      jsonb_build_object('sub', existing_id::text, 'email', target_email),
      'email', now(), now(), now()
    ) on conflict (provider_id, provider) do nothing;

    insert into public.members (id, status, display_name)
    values (existing_id, 'active', target_name)
    on conflict (id) do update set status = 'active', display_name = excluded.display_name;

    raise notice 'Seed: actualizado % (%) -> active, password %', target_email, existing_id, target_password;
  end if;
end
$$;

-- El cliente de la Sala lee `room_snapshot`. 20260922071000 expuso
-- `extensionCount` en `stage_snapshot`, que nadie llama, y 20260922072000
-- reescribió `room_snapshot` desde una copia infiel: llamaba a
-- `is_session_member` (no existe → error 42883) y devolvía jsonb.
--
-- Esta versión parte del cuerpo real volcado de la DB (idéntico a
-- 20260922000000) y solo añade dos líneas: el SELECT del assignment
-- activo trae `extension_count` y el JSON del modo `active` lo emite
-- como `extensionCount`.

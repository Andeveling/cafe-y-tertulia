...
function collect({ yMin, yMax }) {
	...
	const idOf = (el) => {
		if (!el.__lid)
			el.__lid = ++window.__lidCounter || (window.__lidCounter = 1);
		return el.__lid;
	};
	...
	for (const el of document.querySelectorAll("*")) {
		...
		if (parseFloat(c.borderBottomWidth) > 0 && paints(c.borderBottomColor))
			addH(
				r.bottom,
				"border-bottom",
				`${c.borderBottomWidth} ${c.borderBottomColor}`,
			);
		...
		if (parseFloat(c.borderRightWidth) > 0 && paints(c.borderRightColor))
			addV(
				r.right,
				"border-right",
				`${c.borderRightWidth} ${c.borderRightColor}`,
			);
		...
		if (
			c.boxShadow &&
			c.boxShadow !== "none" &&
			paints(c.boxShadow.match(/rgba?\([^)]+\)/)?.[0])
		) {
		...
		if (el.tagName === "CANVAS" && r.width >= 200 && r.height <= 160)
			addH(
				r.top,
				"canvas?",
				`${Math.round(r.width)}x${Math.round(r.height)} — inspect pixels`,
			);
		...
		for (const pseudo of ["::before", "::after"]) {
			...
			addH(
				...
				pseudo,
				bar
					? `${pc.height} ${pc.backgroundColor}`
					: `border ${pc.borderTopWidth}/${pc.borderBottomWidth}`,
			);
		}
	}
	return hits;
...
(async () => {
	...
	report(
		"horizontal",
		found.filter((h) => h.axis === "h"),
	);
	report(
		"vertical",
		found.filter((h) => h.axis === "v"),
	);
